...
update_env() {
  ...
  # Write branch-specific URL to application-local.properties so Maven/Flyway/Spring use it
  # (Spring Boot loads this via spring.config.import; without this, tests/migrate use stale/production URL)
  {
...
if [ "$BRANCH" = "main" ] || [ "$BRANCH" = "master" ]; then
  ...
  if [ -z "$TOKEN" ] || [ -z "$EMAIL" ]; then
    echo "Lakebase: could not get credential. Set SPRING_DATASOURCE_* in .env manually."
    exit 0
  fi
...
HOST="$(get_or_create_endpoint "$BRANCH_PATH")"
if [ -z "$HOST" ]; then
  echo "Lakebase: could not get endpoint host for '$LAKEBASE_BRANCH'. Set SPRING_DATASOURCE_* in .env manually."
  exit 0
fi
...
get_credential "${BRANCH_PATH}/endpoints/primary"
if [ -z "$TOKEN" ] || [ -z "$EMAIL" ]; then
  echo "Lakebase: could not get credential. Set SPRING_DATASOURCE_* in .env manually (host: $HOST)."
  exit 0
fi

update_env "$HOST" "$EMAIL" "$TOKEN" "$LAKEBASE_BRANCH"
echo "Lakebase: branch '$LAKEBASE_BRANCH' ready. Updated .env with SPRING_DATASOURCE_*."
