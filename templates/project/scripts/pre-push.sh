#!/usr/bin/env bash
# Pre-push hook: before pushing (e.g. to open or update a PR), ensure repository secrets
...
# Only run set-repo-secrets if we have the three vars (and optionally GITHUB_TOKEN for API path)
