...
export function runScenario(ctx: ScenarioContext): void {
  ...
  describe('Phase D: Verification', function () {
    this.timeout(60000);
    before(function () { if (phaseAFailed) { this.skip(); } });
    it('D1: V5 applied', () => { assert.ok(verifyMigrationApplied(ctx, '5')); });
    it('D2: cart table exists', () => { assert.ok(verifyTableExists(ctx, 'cart')); });
    it('D2: cart_item table exists', () => { assert.ok(verifyTableExists(ctx, 'cart_item')); });
