...
- **Template `pr.yml` honors `LAKEBASE_BASE_BRANCH` as CI fork source** — added a `LAKEBASE_BASE_BRANCH` repo-variable override that, when set (e.g. to `staging`), is used as the parent for the `ci-pr-<N>` Lakebase branch regardless of where the PR targets. Matches the convention the post-checkout hook and `.env.example` already use for local dev: `LAKEBASE_BASE_BRANCH=staging` → "features fork from staging, always". Unset = fall back to PR's `base.ref` (unchanged behavior). Schema-diff step uses the same resolved source so diffs always compare against the actual fork parent.
